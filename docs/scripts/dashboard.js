...
async function fetchLocations() {
    try {
        ...
        const locations = await Promise.all(filteredThings.map(async thing => {
            ...
            //if (thing.name === 'box_gmr_twl-box_0924008') anzeigeName = 'Test Box 2 (nicht anzeigen)';
            return {
                id: thing['@iot.id'],
                name: thing.name,
                ...
                if (obsTide && obsTide.length > 0) {
                    ...
                    if (filteredObsTide.length > 0) {
                        datasets2.push({
                            ...
                            data: filteredObsTide.map(o => ({x: o.phenomenonTime, y: o.result}))
                        });
                    }
                }
            }
                ...
                if (obsStd && obsStd.length > 0) {
                    ...
                    if (filteredObsStd.length > 0) {
                        datasets2.push({
                            ...
                            data: filteredObsStd.map(o => ({x: o.phenomenonTime, y: o.result}))
                        });
                    }
                }
            }
                ...
                if (obsWave && obsWave.length > 0) {
                    ...
                    if (filteredObsWave.length > 0) {
                        datasets2.push({
                            ...
                            data: filteredObsWave.map(o => ({x: o.phenomenonTime, y: o.result}))
                        });
                    }
                }
            }
